...
*This file deals with outliers and changes in consolidation in the 1930s in the lognormal interpolation 
...
gen 	outlier = .
replace outlier = 1 if sector_main == "Agriculture" & year == 1977
replace outlier = 1 if sector_main == "Agriculture" & year == 1978
replace outlier = 1 if sector_main == "Construction" & year == 1978

*===== Step 2: Drop concentration estimates for these years =========

foreach l of varlist tsh_assets_ln* {
    local vlist "`vlist' `l'"
}
...
*===== Step 3: Interpolate concentration for interpolated series ====
* Gap is always shorter than 5 years 
...
foreach v of local vlist {
    ...
    rename 			`v' old_`v'
    sort 			sector_ID year
    by sector_ID: 	ipolate old_`v' year, gen(`v') 
}
drop old*
